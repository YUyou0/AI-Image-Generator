...
const updateImageCard = (imgIndex, imgUrl) => {
  ...
  if (!imgCard) return; // Check if the card exists
...
const generateImages = async (
  ...
  // Full API: https://router.huggingface.co/hf-inference/models/black-forest-labs/FLUX.1-dev
  const MODEL_URL = `https://router.huggingface.co/hf-inference/models/${selectedModel}`;
  const { width, height } = getImageDimensions(aspectRatio);
  generateBtn.setAttribute("disabled", "true"); // Disable generate button during image generation

  // Create an array of image generating promises
  const imagePromises = Array.from({ length: imageCount }, async (_, i) => {
    ...
    try {
      ...
      imgCard.querySelector(".status-text").textContent.content =
        "Generation Failed! Check console for more details.";
    }
  });
  await Promise.allSettled(imagePromises); // Wait for all image generation promises to settle
...
// Fill the prompt input with a random example prompt
